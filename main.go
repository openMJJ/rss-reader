...
func fetchRSS(url string) ([]byte, error) {
	client := resty.New()
	// 模拟Chrome请求
	reqClient := client.R().ImpersonateChrome()
	...
	if resp.StatusCode() != 200 {
		log.Printf("Failed to fetch RSS: %s", url)
		return nil, err
	}
...
func updateFeedData(url string) {
	// 获取RSS数据
	data, err := fetchRSS(url)
	if err != nil {
		log.Printf("Error fetching RSS data: %v", err)
		return
	}

	// 解析并存储数据
	parsedData, err := utils.ParseRSSData(data)
	if err != nil {
		log.Printf("Error parsing RSS data: %v", err)
		return
	}

	// 存储到数据库或缓存
	globals.Lock.Lock()
	globals.DbMap[url] = parsedData
	globals.Lock.Unlock()
}
